...
import { GetGallerySection, UpdateGallerySection } from '@/services/Sections/GallerySectionService';

export default function GallerySectionManagementPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsSaving(true);
        try {
            await UpdateGallerySection({
                id: formData.id,
                title: formData.title.trim(),
                description: formData.description.trim()
            });

            alert("Galeri bölümü başarıyla güncellendi!");
            ...
            <div className="max-w-5xl mx-auto">
                <div className="mb-8">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-slate-900">Galeri Bölümü Başlık Yönetimi</h1>
                            <p className="text-slate-500 mt-1">Ana sayfanızdaki fotoğraf galerisi alanının üst başlık ve alt açıklama metinlerini buradan yönetebilirsiniz.</p>
                        </div>
                        <div className="flex gap-3">
                            <Link href="/#tur-fotograflari" target="_blank" className="inline-flex items-center gap-2 px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 font-medium transition-colors text-sm">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    {isLoading ? (
                        <div className="p-12 text-center text-slate-500 font-medium animate-pulse">Mevcut galeri başlık bilgileri getiriliyor, lütfen bekleyiniz...</div>
                    ) : (
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            <div>
                                <label htmlFor="title" className="block text-sm font-medium text-slate-700 mb-2">Galeri Bölüm Başlığı (Title)</label>
                            ...
                            <div>
                                <label htmlFor="description" className="block text-sm font-medium text-slate-700 mb-2">Galeri Bölüm Açıklaması (Description)</label>

...
export default function AboutCardsManagementPage() {
    const [aboutCards, setAboutCards] = useState([
        { id: 1, title: 'Geniş Araç Filosu' },
        { id: 2, title: 'Deneyimli Rehber Kadrosu' },
        { id: 3, title: '7/24 Kesintisiz Destek' },
    ]);

    const handleDelete = async (id) => {
        const confirmed = window.confirm('Bu kartı silmek istediğinize emin misiniz kanka?');
        if (!confirmed) return;
        ...
        try {
            ...
            console.error('Silme işlemi sırasında bir şeyler zortladı:', error);
        }
    };
                ...
                <div className="grid grid-cols-1 gap-4 md:hidden">
                    ...
                    {aboutCards.length === 0 && (
                        <div className="bg-white border border-slate-200 p-6 rounded-xl text-center text-sm text-slate-400 font-medium">
                            Henüz hiç hakkımızda kartı eklenmemiş kanka.
                        </div>
                    )}
                </div>
